class SuperAdmin::DashboardController < SuperAdmin::SuperAdminController
  ...
  def index
    ...
    @data_delivery = delivery_total_data(date_range)
    @data_bill = delivery_amount_data(date_range)
    @data_brand = brand_revenue(date_range)
  ...
  def preview_pdf_file
    ...
    data_brand = brand_revenue(date_range)
    url_chart_brand = "https://quickchart.io/chart?c=#{CGI.escape(data_brand.to_json)}"
    pdf_html = ActionController::Base.new.render_to_string(template: 'pdfs/super_admin/dashboard.html.erb',
                                                           layout: 'pdf',
                                                           locals: {
                                                             url_chart_delivery: url_chart_delivery,
                                                             url_chart_bill: url_chart_bill,
                                                             url_chart_brand: url_chart_brand
                                                           },
